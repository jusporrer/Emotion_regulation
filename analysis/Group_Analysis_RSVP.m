...
for subj_idx = 1:length(subject_ID) 
    ...
    rsvp(subj_idx) = Individual_Analysis_Memory(subject_ID(subj_idx), false); 
end

perf = mean([rsvp.performance]);
perf_DC = mean([rsvp.perf_DC]);
perf_CC = mean([rsvp.perf_CC]);
perf_BC = mean([rsvp.perf_BC]);

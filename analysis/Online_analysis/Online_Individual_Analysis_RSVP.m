...
load('data_rsvp.mat','data_rsvp');


%% =================== Initialise                       ===================

nTrain                      = 24;
training                    = data_rsvp(1:24,:); 
test_part                   = data_rsvp.test_part(nTrain+1:end);
test_part                   = cellstr(test_part);
%(1 = Small reward, 2 = Large reward)
reward                      = data_rsvp.condiRwd(nTrain+1:end);
%(1 = DC_male, 2 = DC_female, 3 = CC_male, 4 = CC_female, 5 = BC_male , 6 = BC_female)
condition                   = data_rsvp.condiEmoTrial(nTrain+1:end);
block                       = data_rsvp.blockNb(nTrain+1:end);
trial                       = data_rsvp.trialNb(nTrain+1:end);
rt                          = data_rsvp.rt(nTrain+1:end);
% (1 = femquest, 2 = homquest)
%%instQuest                   = [data_rsvp(nTrain+1:end).instQuest];
% (79 = [o] / oui; 78 = [n] / non)
response                    = data_rsvp.key_press(nTrain+1:end);
posCritDist                 = data_rsvp.posCritDist(nTrain+1:end);
posTarget                   = data_rsvp.posTarget(nTrain+1:end);
...
correct_Fem                 = (response(nTrial) == 79 & instQuest(nTrial) == 1 ...                           % all fem targets
    & (target(nTrial) == 0 | target(nTrial) == 2));
% correct_FearFem             = (response(nTrial) == 79 & instQuest(nTrial) == 1 ...                           % fearful fem targets
%     & target(nTrial) == 0);
% correct_NeutralFem          = (response(nTrial) == 79 & instQuest(nTrial) == 1 ...                       % neutral fem targets
%     & target(nTrial) == 2);

% Correctly detect hom when hom was target
correct_Hom                 = (response(nTrial) == 79 & instQuest(nTrial) == 2 ...                           % all hom targets
    & (target(nTrial) == 1 | target(nTrial) == 3));
% correct_FearHom             = (response(nTrial) == 79 & instQuest(nTrial) == 2 ...                           % fearful hom targets
%     & target(nTrial) == 1);
% correct_NeutralHom          = (response(nTrial) == 79 & instQuest(nTrial) == 2 ...                       % neutral hom taregts
%     & target(nTrial) == 3);

% Correctly reject fem when hom was target
correct_NotFem              = (response(nTrial) == 78 & instQuest(nTrial) == 1 ...
    & (target(nTrial) == 1 | target(nTrial) == 3));
% correct_NotFem_FearHom      = (response(nTrial) == 78 & instQuest(nTrial) == 1 ...
%     & target(nTrial) == 1);
% correct_NotFem_NeutralHom   = (response(nTrial) == 78 & instQuest(nTrial) == 1 ...
%     & target(nTrial) == 3);

% Correctly reject hom when fem was target
correct_NotHom              = (response(nTrial) == 78 & instQuest(nTrial) == 2 ...
    & (target(nTrial) == 0 | target(nTrial) == 2));
% correct_NotHom_FearFem      = (response(nTrial) == 78 & instQuest(nTrial) == 2 ...
%     & target(nTrial) == 0);
% correct_NotHom_NeutralFem   = (response(nTrial) == 78 & instQuest(nTrial) == 2 ...
%     & target(nTrial) == 2);
...
disp(['Performance : ',num2str(round(rsvp.performance)), ...
    '% correct trials with ',num2str(round(rsvp.hit_rate*100)), '% hits & ', ...
    num2str(round(rsvp.reject_rate*100)), '% rejections' ]);

%% =================== Performance - Incorrect Trials   ===================

% Incorrectly not detect fem when fem was target ( false alarm: says present when not present)
incorrect_Fem               = (response(nTrial) == 78 & instQuest(nTrial) == 1 ...                             % all fem targets
    & (target(nTrial) == 0 | target(nTrial) == 2));
% incorrect_FearFem           = (response(nTrial) == 78 & instQuest(nTrial) == 1 ...                         % fearful fem targets
%     & target(nTrial) == 0);
...
incorrect_Hom               = (response(nTrial) == 78 & instQuest(nTrial) == 2 ...                             % all hom targets
    & (target(nTrial) == 1 | target(nTrial) == 3));
% incorrect_FearHom           = (response(nTrial) == 78 & instQuest(nTrial) == 2 ...                         % fearful hom targets
%     & target(nTrial) == 1);
% incorrect_NeutralHom        = (response(nTrial) == 78 & instQuest(nTrial) == 2 ...                      % neutral hom taregts
%     & target(nTrial) == 3);

% Incorrectly reject hom when hom was target ( miss: says not preset when present)
incorrect_NotFem            = (response(nTrial) == 79 & instQuest(nTrial) == 1 ...
    & (target(nTrial) == 1 | target(nTrial) == 3));
% incorrect_NotFem_FearHom    = (response(nTrial) == 79 & instQuest(nTrial) == 1 ...
%     & target(nTrial) == 1);
% incorrect_NotFem_NeutralHom = (response(nTrial) == 79 & instQuest(nTrial) == 1 ...
%     & target(nTrial) == 3);

% Incorrectly reject fem when fem was target
incorrect_NotHom            = (response(nTrial) == 79 & instQuest(nTrial) == 2 ...
    & (target(nTrial) == 0 | target(nTrial) == 2));
% incorrect_NotHom_FearFem    = (response(nTrial) == 79 & instQuest(nTrial) == 2 ...
%     & target(nTrial) == 0);
% incorrect_NotHom_NeutralFem = (response(nTrial) == 79 & instQuest(nTrial) == 2 ...
%     & target(nTrial) == 2);
...
disp(['Performance : ',num2str(round(rsvp.perf_incorrect)), ...
    '% incorrect trials with ',num2str(round(rsvp.falseAlarm_rate*100)), '% false alarms & ', ...
    num2str(round(rsvp.miss_rate*100)), '% misses' ]);
...
if sum(DC_hom) == sum(DC_fem) == sum(CC_hom) == ...
    ...
    disp(['All conditions have ',num2str(50), 'trials']);
else
    disp(['Warning, not all condition have ',num2str(50), ' trials']);
end

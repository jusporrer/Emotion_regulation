...
for i = 1: setSize/4
    display(posFF)
    img(floor(posFF(i,2)+1:posFF(i,4)),floor(posFF(i,1)+1:posFF(i,3)),:) = fearFem{i};
    img(floor(posNF(i,2)+1:posNF(i,4)),floor(posNF(i,1)+1:posNF(i,3)),:) = neutralFem{i};
    img(floor(posFM(i,2)+1:posFM(i,4)),floor(posFM(i,1)+1:posFM(i,3)),:) = fearMale{i};
    img(floor(posNM(i,2)+1:posNM(i,4)),floor(posNM(i,1)+1:posNM(i,3)),:) = neutralMale{i};
end 
end 
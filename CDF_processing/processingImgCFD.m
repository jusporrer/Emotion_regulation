...
%% Import the CSV file 
CFD_norming_data_original = import_normative_data_CFD("..\CFD Version 2.0.3\CFD 2.0.3 Norming Data and Codebook.xlsx", "CFD 2.0.3 Norming Data", [6, 602]);
...
remove_non_caucasian = [];
 for nbimg = 1: size(CFD_norming_data,1)
     if CFD_norming_data.Race(nbimg) == 'A' || ...
             CFD_norming_data.Race(nbimg) == 'L' || ...
             CFD_norming_data.Race(nbimg) == 'B'
         remove_non_caucasian = [remove_non_caucasian, nbimg];
     end 
 end 

 disp([' - ',num2str(length(remove_non_caucasian)),' people were removed because they were not caucasian.'])
...
std_Unusual  = std(CFD_norming_data.Unusual)*3 ; %  std from the mean
...
remove_Surprised = []; 
 ...
 remove = [remove_non_caucasian,remove_unusual,remove_Afraid, remove_Surprised]; 
...
initial_folder = '../CFD Version 2.0.3/CFD 2.0.3 Images';
